...
# Defining string input to its respective mathematical operations (only + and -)
ops = {
    "+": operator.add,
    "-": operator.sub
}
...
def get_combinations(num1, op1, op2):
    ...
    :param op2: Second operator of the equation
...
def validate_input(list1):
    ...
    if len(list1) == 3:
        if list1[0] in operands and list1[1] in operands:
            ...
            print("Invalid operator input.!! Accepted are +, -")
            exit()
    else:
...
abc_combinations = get_combinations(int(row_1[2]), '+','-')
# print(abc_combinations)
pqr_combinations = get_combinations(int(row_2[2]), '+', '+')
xyz_combinations = get_combinations(int(row_3[2]), '-', '+')
apx_combinations = get_combinations(int(col_1[2]), '+','-')
bqy_combinations = get_combinations(int(col_2[2]), '-', '+')
crz_combinations = get_combinations(int(col_3[2]), '+', '+')
